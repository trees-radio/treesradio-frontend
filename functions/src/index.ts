...
export const cleanupExpiredChatImages = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          if (metadata &&
            ...
            const expirationTime = parseInt(String(metadata.metadata.expires_at), 10);

            // If the file has expired, add it to our delete queue
            if (expirationTime <= currentTimestamp) {
              console.log(`Deleting expired file: ${file.name}, expired at: ${new Date(expirationTime * 1000).toISOString()}`);
            ...
            if (metadata.timeCreated) {
              const fileCreationTime = new Date(metadata.timeCreated).getTime() / 1000;
              const sixHoursInSeconds = 6 * 60 * 60;

              if (currentTimestamp - fileCreationTime >= sixHoursInSeconds) {
                console.log(`Deleting old file without expiration: ${file.name}, created: ${metadata.timeCreated}`);
                deletePromises.push(file.delete());
                deletedCount++;
              }
            }
      ...
      console.log(`Cleanup complete. Deleted ${deletedCount} expired chat images.`);
      return null;
    } catch (error) {
 ...
 * This is a backup cleanup mechanism in case the scheduled job misses some images
 */
      ...
      if (message && message.msg) {
        ...
        if (imageUrlMatch && imageUrlMatch[1]) {
          ...
          if (storagePathMatch) {
            ...
            if (exists) {
              await file.delete();
              console.log(`Deleted image ${storagePath} associated with deleted chat message ${context.params.messageId}`);
            }
          }
        }
      }
...
export const generateChatImageStats = functions.pubsub
  ...
  .onRun(async (context) => {
    try {
      ...
      let countByUser: { [userId: string]: number } = {};
      ...
      const stats: ImageStats = {
        ...
        oldestFile: oldestTimestamp !== Infinity ? new Date(oldestTimestamp * 1000).toISOString() : null,
        newestFile: newestTimestamp !== 0 ? new Date(newestTimestamp * 1000).toISOString() : null,
